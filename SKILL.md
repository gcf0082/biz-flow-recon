...
- **mermaid 示意图**（**仅用 `flowchart TD`**，禁用 sequenceDiagram / stateDiagram / graph LR）。规则：
  ...
  - **图必涵盖所有文件/命令/外呼/加解密操作**——每项独立节点；标签含动作 + 完整路径或 URL（动态部分 `{var}`）+ `<br/><small>文件:行号</small>`
  - 业务步骤名 + 技术动作可同节点呈现（如 `① 落地原始文件\n写 /data/uploads/{filename}`）
  - 用户输入来源、关键控制点判断（`{...}` 菱形）应纳入图
  - 循环用 `subgraph "对每个 X："`，分支用菱形 + 边标 `|是|/|否|`，gate 失败画到"异常终止"subgraph
  - 与安全无关的通用控制流（5xx 错误页、通用异常包装）属噪声，不画
  - 每接口至多一张图
- **编号步骤**：内在顺序操作
- **字段化短列表**：`**第三方**:` 等，仅列实际发生
- **连贯叙述**：事实自然成句时
