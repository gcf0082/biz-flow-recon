...
- **项目根 `biz-flow-recon/`**：仅留 `templates/`（项目级覆盖，可选）
...
`.opencode/skills/biz-flow-recon/{SKILL.md,knowledge/,templates/,docs/}` 与 `.opencode/agents/`、`biz-flow-recon/templates/`、`_results/` 目录名为**固定约定**。
...
- **始终覆盖**：aggregator-writer / completion-verifier 每次都覆盖最新（基于当前 _results/ 全貌重写）
