...
- **skill 仓本身**：通用方法论 + 默认模板（`.opencode/skills/biz-flow-recon/`）+ 子代理定义（`.opencode/agents/`）
- **被分析项目内**：约定一个 `biz-flow-recon/` 子目录存放该项目的知识库、模板覆盖与产物；opencode runtime 还需要 `.opencode/skills/` 与 `.opencode/agents/`
...
<被分析项目>/
├── biz-flow-recon/
│   ├── knowledge/                # 可选：项目专属知识库（**所有子代理默认加载**）
...
│       └── _audit.md
└── .opencode/                    # 可选（仅 opencode runtime）
    ├── skills/biz-flow-recon/    # SKILL.md / templates/ / docs/
    └── agents/                   # 7 份子代理 .md
```

`biz-flow-recon/` 与 `.opencode/{skills,agents}/` 目录名为**固定约定**。

## 知识库（knowledge/）

**所有 7 个子代理在执行前默认完整读取本目录所有 .md** 作为先验上下文——这是项目特有说明的标准位，给项目结构、术语、约定、内部服务定位等关键线索；agent 自行读，**不依赖主 agent 转发**（避免上下文丢失）。建议存放：

- **`briefing.md`**：**项目高层说明**——技术栈与版本、模块布局总览、外部接口风格约定（路径命名、版本前缀、错误码规范）、跨模块共用机制（统一鉴权 / trace / 异常）、特殊查找线索。是给 agent 的项目级 brief，让其搜索与判断更准确。
- **`glossary.md`**：项目特有术语
- **`conventions.md`**：团队约定（鉴权方式、加密库、日志规范）+ skill 行为开关（见下）
- **`modules/*.md`**：每个子模块一份说明
- **`auto-*.md`**：`knowledge-extractor` 自动写入（含 `auto-briefing.md` 初稿）；与上述用户文件**并存且不覆盖**
...
产物结构 = 每接口一份独立文件 + 两个横向产物（`interfaces.md` / `outbound.md`）+ aggregator 索引 + `_audit.md`。
