...
│   │   ├── templates/            # 默认输出模板
│   │   ├── docs/                 # 工作目录约定文档
│   │   └── knowledge/            # 项目专属先验（除 aggregator-writer 外强制加载）
...
**除 `aggregator-writer` 外的子代理在执行前默认完整读取本目录所有 .md** 作为先验上下文——这是项目特有说明的标准位，给项目结构、术语、约定、内部服务定位等关键线索；agent 自行读，**不依赖主 agent 转发**（避免上下文丢失）。`aggregator-writer` 仅做索引拼装，跳过本目录。建议存放：
