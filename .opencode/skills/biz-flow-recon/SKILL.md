...
- **代码引用 / 路径 / URL / 命令一律原样保留**：产物中所有源码引用（`path:line`）、文件系统路径、HTTP / RPC URL、命令行、SQL 等**不论长度都不省略、不截断、不用 `…` / `...` / 中段缩写**。表格列宽不够就让单元格 wrap，mermaid 节点写不下就多行 `<br/>`——绝不删字符。视觉缩写**只许**用在纯展示项（章节标题、子功能名等），**永不**用于"可被 grep / 复制定位的标识符"。动态片段保留 `{变量名}` 占位是**结构化变量标注**，不属于省略。
