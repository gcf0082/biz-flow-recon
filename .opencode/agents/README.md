...
**除 `aggregator-writer` 外的子代理执行前默认加载** `<被分析项目>/.opencode/skills/biz-flow-recon/knowledge/` 下全量 `.md`（`briefing.md` / `glossary.md` / `conventions.md` / `modules/*.md`）作为项目级先验上下文。子代理自行读取，不依赖主 agent 转发。`aggregator-writer` 仅做索引拼装，跳过 knowledge/。
