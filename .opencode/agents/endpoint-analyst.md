...
> **图自包含为主**——把所有事实优先嵌入图节点标签（输入流向、关键控制点 metadata、硬编码标注、文件/命令/外呼操作等都进图）；**图能承载的下方 labeled 列表不再补充**，仅保留图无法承载的细节（DTO 全限定名、请求体字段树、跨节点的复杂规则汇总）。理想情况下下方文字仅剩 `**请求**` 一行 + 必要时 `## 未能追溯的引用` 节。
...
- **字段化短列表**：仅在图无法承载时使用——通常只剩 `**请求**`（DTO + 字段树）
- **连贯叙述** / **一句话**：极简接口
